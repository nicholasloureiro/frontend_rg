...
import {
  ...
  CanalOrigemChart,
  AluguelVendaValorChart,
  AluguelVendaQuantidadeChart,
  AluguelVendaTicketMedioChart,
...
const Home = () => {
  ...
  const handleFilterChange = (newFilters) => {
    ...
    changePeriod(
      ...
      customDate,
      customType,
      newFilters.atendente,
      newFilters.tipoCliente,
      newFilters.formaPagamento,
      newFilters.canalOrigem,
  ...
  const processResultadosData = (resultadosData) => {
    if (!resultadosData)
      return { total_pedidos: 0, total_recebido: 0, numero_pedidos: 0 };
      ...
      <div className="home">
        ...
        <div className="dashboard-grid">
          {statusData.map((section) => (
            <div className="dashboard-section" key={section.section}>
              <h3 style={{ marginBottom: 12, color: section.color }}>
                {section.section}
              </h3>
            ...
            <div className="kpi-card">
              ...
              <p className="kpi-value">
                {new Intl.NumberFormat("pt-BR", {
                  style: "currency",
                  currency: "BRL",
                }).format(dashboardData?.kpis?.total_recebido || 0)}
              </p>
            </div>
            <div className="kpi-card">
              ...
              <p className="kpi-value">
                {new Intl.NumberFormat("pt-BR", {
                  style: "currency",
                  currency: "BRL",
                }).format(dashboardData?.kpis?.total_vendido || 0)}
              </p>
            </div>
            <div className="kpi-card">
              <p className="kpi-label">Total de Atend.</p>
              <p className="kpi-value">
                {dashboardData?.kpis?.total_atendimentos || 0}
              </p>
            </div>
            <div className="kpi-card">
              <p className="kpi-label">Atnd fechados</p>
              <p className="kpi-value">
                {dashboardData?.kpis?.atendimentos_fechados || 0}
              </p>
            </div>
            <div className="kpi-card">
              <p className="kpi-label">Atnd não fech.</p>
              <p className="kpi-value">
                {dashboardData?.kpis?.atendimentos_nao_fechados || 0}
              </p>
            </div>
            <div className="kpi-card">
              <p className="kpi-label">Tx. Conv</p>
              <p className="kpi-value">
                {(dashboardData?.kpis?.taxa_conversao || 0).toFixed(2)}%
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="charts-grid">
          ...
          <div className="chart-section">
            ...
            >
              <div>
                ...
                <TaxaConversaoChart
                  data={dashboardData?.atendentes_taxa_conversao || []}
                />
              </div>
              <div>
                <h4
          ...
          <div className="chart-section">
            ...
            <h4 style={{ margin: "10px 0", fontSize: "14px", color: "#555" }}>
              Valor total vendido por cada atendente
            </h4>
            <TotalVendidoChart
              data={dashboardData?.atendentes_total_vendido || []}
            />
          </div>
          ...
          <div className="chart-section">
            ...
            >
              <div>
                ...
                <TipoClienteChart
                  data={dashboardData?.grafico_tipo_cliente || []}
                />
              </div>
              <div>
                <h4
          ...
          <div className="chart-section">
            ...
            <h4 style={{ margin: "10px 0", fontSize: "14px", color: "#555" }}>
              Quantidade de atendimentos fechados por canal
            </h4>
            <CanalOrigemChart
              data={dashboardData?.grafico_canal_origem || []}
            />
          </div>

          {/* Gráfico de Aluguel/Venda */}
          <div className="chart-section">
            <h3>Análise de Aluguel/Venda</h3>
            <div>
              <h4
                style={{
                  margin: "0 0 10px 0",
              ...
              <AluguelVendaValorChart
                data={dashboardData?.grafico_aluguel_venda || []}
              />
            </div>
          </div>
          <div className="chart-section">
            <h3 style={{color: 'transparent', userSelect: "none"}}>Análise de Aluguel/Venda</h3>
            <div>
              <h4
                style={{
                  margin: "0 0 10px 0",
